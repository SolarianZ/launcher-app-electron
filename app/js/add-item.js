document.addEventListener('DOMContentLoaded', () => {
    // DOM元素
    const modalTitle = document.querySelector('.modal-title-bar'); // 获取标题栏元素
    ...
    itemPathInput.addEventListener('input', async () => {
        ...
        if (path) {
            ...
            } else if (path.includes('/') || path.includes('\\') || path.match(/^[a-zA-Z]:\\/)) {
                try {
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            window.electronAPI.closeAddItemWindow();
        } else if (e.key === 'Enter' && !addBtn.disabled) {
            addBtn.click();
        }
    });
});