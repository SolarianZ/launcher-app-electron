...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    showEditItemDialog: (item, index) => ipcRenderer.send('show-edit-item-dialog', { item, index }),

    // 获取项目类型
    getItemType: (path) => ipcRenderer.invoke('get-item-type', path),

    // 项目操作
    updateItem: (index, updatedItem) => ipcRenderer.invoke('update-item', { index, updatedItem }),
