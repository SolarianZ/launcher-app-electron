const { app, BrowserWindow, ipcMain, shell, clipboard, Menu, Tray, globalShortcut, nativeImage } = require('electron');
...
function createTray() {
    // 使用自动缩放的图标
    const iconPath = path.join(__dirname, 'app', 'assets', 'icons', 'tray-icon.png');
            ...
            */
            ...
            try {
                if (platform === 'win32') {
                    ...
                    if (/^\/[KC]/i.test(item.path)) { // 若已经包含 /K 或 /C 则直接执行
                    ...
                    exec(`osascript -e 'tell app "Terminal" to do script "${escapedCommand}"'`);
                } else {
                    // Linux - 尝试打开各种常见终端
                    const terminals = [
                    ...
                    // 替换命令
                    const command = terminals[0].replace('{CMD}', item.path.replace(/"/g, '\\"'));
                    exec(command, (error) => {
                        if (error) {
